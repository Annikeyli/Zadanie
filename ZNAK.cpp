...
#include "zodiak.h"
Znak::Znak() {}
Znak::Znak(const char* n, const char* s,Zodiak z, int day, int month, int year) {// íŕç ęîíńňđóęňîđ, ěĺňîä, íĺń¸ň čě˙ ęëŕńńŕ, íč÷ĺăî íĺ âîçâđŕůŕĺň
    strcpy_s(surname, s);// ôóíęöč˙ äë˙ ęîďčđîâŕíč˙ ńčěâîëîâ čç îäíîé ńňđîęč â äđóăóţ ; ăäĺ őđŕíčňń˙ ńęîďčđ ńňđîęŕ, čńőîäíŕ˙ ńňđîęŕ
    strcpy_s(name, n);
    zodiak = z;
    data[0] = day;
    data[1] = month;
